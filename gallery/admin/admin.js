...
async function createThumb(file) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    // 必须 revoke：一次批量上传 50 张原图 = 50 个永不释放的 blob 引用
...
async function loadPhotos(reset) {
  ...
  // 给按钮加 disabled，免得用户在加载中以为没响应又狂点
...
document.getElementById("deleteSelectedBtn").addEventListener("click", async () => {
  ...
  // 失败的卡片保留选中状态，让用户能看到/重试；只清成功的
  let okCount = 0, failCount = 0;
  keys.forEach((key, i) => {
    if (results[i]) {
      document.querySelector(`.photo-card[data-key="${CSS.escape(key)}"]`)?.remove();
      selected.delete(key);
      okCount++;
    } else {
      failCount++;
  ...
  updateSelUI();
  const remaining = document.getElementById("photo-grid").children.length + " photos loaded";
  setStatus(failCount > 0 ? `${remaining} (${failCount} delete failed)` : remaining);
});
...
function openCaptionModal(key) {
  ...
  const zhEl = document.getElementById("caption-zh");
  const enEl = document.getElementById("caption-en");
  zhEl.value = "";
  enEl.value = "";
  document.getElementById("caption-modal").classList.add("open");
  // 打开就 focus 中文 textarea —— 键盘流不必先 Tab 一下才能开始打字
  zhEl.focus();
...
// Esc 关闭 modal —— 没这个键盘用户只能找鼠标
document.addEventListener("keydown", e => {
  if (e.key === "Escape" && document.getElementById("caption-modal").classList.contains("open")) {
    closeCaptionModal();
  }
});
