...
(function () {
  ...
  function openMoreOverlay(AS, games, onCloseFocusTarget) {
    ...
    function close() {
      ...
      // Restore focus to the trigger so keyboard users don't get dumped on <body>.
      // Skipped only when a game modal is about to open (it'll steal focus anyway).
      if (!_skipFocusRestore && onCloseFocusTarget && typeof onCloseFocusTarget.focus === "function") {
    ...
    function onListClick(e) {
      ...
      // Skip close()'s focus restore — openGameModal will grab focus next.
  ...
  function mountAll() {
    ...
    document.querySelectorAll(".arcade-shelf-mount").forEach(function (mount) {
      ...
      if (previewCount !== null && sorted.length > previewCount) {
        ...
        } else if (moreBtn && !shelfHeader) {
          // arcade-shelf changed its DOM and we can't find the header to inject
          // the More button into — log so we notice instead of silently hiding it.
          console.warn("arcade-shelf-init: .arcade-shelf-card-header not found; More button stays hidden");
        }
      }
    });
  }
